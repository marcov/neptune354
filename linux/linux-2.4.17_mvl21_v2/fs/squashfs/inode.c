...
#ifdef __TI_SQUASHFS_LZMA
        if(!(msblk->lzma_data = (char *) vmalloc(lzma_workspace_size()))) {
                ERROR("Failed to allocate lzma workspace\n");
                goto failed_mount;
        }
        lzma_init(msblk->lzma_data, lzma_workspace_size());
#else
...
static int __init init_squashfs_fs(void)
{
	...
	printk(KERN_INFO "squashfs: version 3.1 (2006/08/19) "
		"Phillip Lougher, lzma support by McMCC\n");

...
start_lan(void)
{
	...
	if ((s = socket(AF_INET, SOCK_RAW, IPPROTO_RAW)) < 0)
		return;
	cprintf("===>Starting Lan interfaces:\n");
	...
	if (strncmp(lan_ifname, "br", 2) == 0) {
		...
		/*for each interface  Loop*/
		foreach(name, lan_ifnames, next) {
				
			cprintf("Interface name=[%s] Bridged if name=[%s]\n", name, lan_ifname);
			/* Bring up interface */
			...
			if (ioctl(s, SIOCGIFHWADDR, &ifr) == 0 &&
				...
				if (ioctl(s, SIOCGIFHWADDR, &ifr) == 0) {
						...
						fprintf(stderr,"Setting bridgeg interface %s  hwaddr to physical interface %s\n",lan_ifname,name);
				}
				else 
					perror(lan_ifname);
				}
	...
	cprintf("Interface [%s] , IP Address: %s , Netmask: %s\n",lan_ifname, 
...
/* ADSL Polling for detecting line (layer 1)
 * When detected, does signal SIGALRM,
 * handled in rc.c
 * */
int
adslpolling_main(int argc, char **argv)
{
	...
	*/
	...
	cprintf("Detecting ADSL Line (adslpolling_main)...\n");
	...
	while(1)
	{
		...
		if(check_adsl_status())
		{
			if(nvram_match("adsluptime", "0"))
			{
           	...
           	syslog(LOG_INFO,"ADSL Signal Status: %s", "SHOWTIME");
			break;
		}
	}
	
	cprintf("ADSL Line Detected: stopping  polling...\n");
	kill(1, SIGALRM);
	return -1;
}
...
/*
 * Depending on the mode, it calls
 * start_pppoe
 * start_ppppoa
 * 
 * Those functions, when finished, calls "start_wan_done"
 */
void
...
start_wan_done(char *wan_ifname)
{
	...
	*/
	...
	/* Run a script upon connection /mnt/online.sh */
	if ( nvram_match("onsh_enable","1") )
	{
		run_online_sh();
	}
...
void run_online_sh()
{
	cprintf("Starting online script\n");
	eval("sh","/mnt/online.sh");
}

...
{
	int ret;
	cprintf("Stopping dnsmasq: ");
	ret = eval("killall", "dnsmasq");
	cprintf("[OK]\n");
	dprintf("done\n");
	return ret;
}	
...
{
	int ret;
	cprintf("Stopping httpd: ");
	ret = eval("killall", "httpd");
	cprintf("[OK]\n");
	dprintf("done\n");
	return ret;
}
...
//ntpclient tests the wan status... 
int
start_ntpc(void)
{
	int ret;
	char *ntpc_argv[] = {"ntpclient", "-h", "pool.ntp.org","-s", "-c", "1", NULL};
	ret = _eval(ntpc_argv, ">>/dev/null", 1, NULL);
	return ret;
}
...
start_ntpd(void)
{
	int ret,i=0;
	pid_t pid;
	char *ntp_argv[] = { "/sbin/ntpd",  NULL };
	
	cprintf("Starting ntpd\n");
	ret = start_ntpc();
	while (i<10 && ret!=0)
	{
		i++;
		cprintf("[TEST #%d]ntpclient failed!\n",i);
		sleep(1);
		ret = start_ntpc();
	}
...
/*Every service is started using a function call*/
...
#ifdef DROPBEAR_SUPPORT
/* ret = nvram_match(xxx_enable, char *val);
 * if( xxx_enable=1 && val=1 ) => ret=1
 * if( xxx_enable=1 && val=0 ) => ret=0
 * if( xxx_enable=0 && val=1 ) => ret=0
 * if( xxx_enable=0 && val=0 ) => ret=1
 * if( xxx_enable=NOT_FOUND  ) => ret=0 
 *
 */

int start_dropbear(void){
	...
	{
		pid_t pid;
		char * dropbear_port = nvram_get("dropbear_port");
		char * dropbear_argv[] = { "dropbear", "-r", "/mnt/dropbear_rsa_host_key", "-p" , dropbear_port , NULL };
		
		fprintf(stderr,"Starting dropbear on port %s: ", dropbear_port);
	    //ret = eval("dropbear", "-r", "/mnt/dropbear_rsa_host_key");
	    ret = _eval(dropbear_argv, NULL, 0, &pid);
	    cprintf("[OK]\n");
	}
	return ret;
}

int stop_dropbear(void){
	int ret;
	
	cprintf("Stopping dropbear: ");
...
int
start_custom_script(void)
{
	int ret = 0;
	
	if ( nvram_match("cush_enable","1") ){
		cprintf("Starting user custom script: ");
		ret = eval("sh","/mnt/script.sh");
		cprintf("[OK]\n");
	}
	return ret;
}
...
start_SysLog(void)
{
	...
	{
		cprintf("Starting Syslog: ");
		chdir("/sbin");
		eval("syslogd","-C");
		chdir("/");
		
		/*Waiting for the syslogd up*/
		sleep(1);
		...
		if(i == 0)
		{
			...
			syslog(LOG_CRIT,"System cold started!\n");
			closelog();
			openlog("", LOG_CONS | LOG_NDELAY, LOG_AUTHPRIV);
			syslog(LOG_CRIT,"Firmware Version:%s%s\n", CYBERTAN_VERSION, MINOR_VERSION); 
			closelog();
		}
		else
		{
			openlog("", LOG_CONS | LOG_NDELAY, LOG_AUTHPRIV);
			syslog(LOG_CRIT,"System warm started!\n");
			closelog();

...
write_mac(const char *path)
{
	...
	if(strstr(path,"mac.bin")) {
		...
		//fixed insmod write sn fail but nvram keep old value issue
		nvram_set("get_sn", SERIAL_NUMBER_DEFAULT);  
		...
		if(nvram_match("get_sn", SERIAL_NUMBER_DEFAULT))
			prom_unsetenv("eoc_vendor_serialnum");
		else
			prom_setenv("eoc_vendor_serialnum", nvram_safe_get("get_sn"));

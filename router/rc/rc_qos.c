...
int init_application()
{
	...
	sprintf(qos_appport[2],"%s","53");
	sprintf(qos_appport[3],"%s","23");
	sprintf(qos_appport[4],"%s","25");
	sprintf(qos_appport[5],"%s","110");
	sprintf(qos_appport[6],"%s",nvram_safe_get("qos_spec_port0"));
	sprintf(qos_appport[7],"%s",nvram_safe_get("qos_spec_port1"));
	sprintf(qos_appport[8],"%s",nvram_safe_get("qos_spec_port2"));
	sprintf(qos_appport[9],"%s",nvram_safe_get("qos_spec_port3"));	
	return 0;
}
...
/*	fprintf(qos_fd,"\n#### Attach SFQ to each class #####\n");
	...
	/*
	 * Add high priority for ntpd port (123), otherwise time is "not available"...
	 */
	fprintf(qos_fd,"/usr/sbin/tc filter add dev ppp0 parent 1:0 protocol ip prio 1 u32 match ip sport 123 0xffff flowid 1:2\n");
	fprintf(qos_fd,"/usr/sbin/tc filter add dev ppp0 parent 2:0 protocol ip prio 1 u32 match ip sport 123 0xffff flowid 2:10\n");
	fprintf(qos_fd,"/usr/sbin/tc filter add dev ppp0 parent 210:0 protocol ip prio 1 u32 match ip sport 123 0xffff flowid 210:1\n");
	fprintf(qos_fd,"/usr/sbin/tc filter add dev ppp0 parent 1:0 protocol ip prio 1 u32 match ip dport 123 0xffff flowid 1:2\n");
	fprintf(qos_fd,"/usr/sbin/tc filter add dev ppp0 parent 2:0 protocol ip prio 1 u32 match ip dport 123 0xffff flowid 2:10\n");
	fprintf(qos_fd,"/usr/sbin/tc filter add dev ppp0 parent 210:0 protocol ip prio 1 u32 match ip dport 123 0xffff flowid 210:1\n");

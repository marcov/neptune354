...
rc_signal(int sig)
{
	...
	{	
		if (sig == SIGHUP) {
			...
			/*
			 * Signal from adslpolling_main, TIMER handled here.
			 */
...
/* 
 * Adsl finished traing, 
 * Start wan connection calling start_wan in network.c 
 */
int
do_timer(void)
{
	...
	/*
	 *  Have to move start_wan_services here because with qos it fails
	 */
	/*
	if ( nvram_match("qosSW","1") )
	{
		cprintf("Starting WAN Services [with QoS enabled].\n");
		start_wan_service();
	}
	*/
	return 0;
...
/* Main loop 
 * 
 * This is the main loop of rc.
 * 
 * It consist of this sequence
 * 
 * 1) sysinit : mount the proc and ramfs, create some /tmp folders
 * 
 * */
static void
main_loop(void)
...
#include <flash_layout.h>
	...
	for (;;) 
	{
		switch (state) 
		{
			//check adsl active
			case POLLING:	
				dprintf("Rc status: POLLING\n");
				start_polling_check();
	            state = IDLE;
				break;
			
			case TIMER:
				dprintf("Rc status: TIMER\n");
				do_timer();
	            state = IDLE;
				break;
			
			case USER: // Restart single service from WEB of tftpd, by honor
	            cprintf("Rc status: USER1 (start single service)\n");
	            start_single_service();
	            state = IDLE;
	            break;
			
			case RESTART:
				cprintf("Rc status: RESTART, now STOP and START\n");
				//cprintf("hello ap!\n");
				/* Fall through */
			
			case STOP:
				cprintf("Rc status: STOP\n");
#ifdef CONFIG_DSL_MODULA_SEL
				...
				//stop_firewall();
				//start_firewall();
		        if (nvram_match("nfsmount_enable","1"))
				{
					cprintf("Unmounting NFS ");
					system("umount /opt");
				}		
		        
		        if (nvram_match("rwpart_enable","1"))
				{
					cprintf("Unmounting /dev/mtdblock/4: ");
			 ...
			 ********************/	
			case START:
				cprintf("** Rc status: START\n");
				eval("sh", "/etc/optimize.sh");
				//Useless
				diag_led_ctl(SYS_READY);
				start_resetbutton();
								
				//WAN UPGRADE NOTIFY USER
#ifdef WAN_UPGRADE
				...
				else
				{
					...
					if (nvram_match("auto_search_ip","1") && nvram_match("lan_detect_dhcp","0") && nvram_match("lan_proto","static"))
					{
						if (firstsetlanip == 1) 
						{	
							cprintf("Firstsetlanip =1; begin to wait...");
							sleep(5);
							cprintf("End waiting\n");
						}
					}
				...
				/*Mount the rw partition*/
				if (nvram_match("rwpart_enable","1"))
				{
					cprintf("Mounting MinixFS: ");
					system("mount -t minix /dev/mtdblock/4 /mnt");
				}
				
				/*Start services in services.c*/
				start_services();
				
				/*Mount nfs folder*/
				if (nvram_match("nfsmount_enable","1"))
				{
					char * ip = (char *)malloc(strlen(nvram_safe_get("nfsmount_ip")) +1);
					char * path = (char *)malloc(strlen(nvram_safe_get("nfsmount_path")) +1);
					char * cmd=NULL;
					sprintf(ip, nvram_safe_get("nfsmount_ip"));
					sprintf(path,nvram_safe_get("nfsmount_path"));
					cmd= (char *)malloc ( strlen(ip) + strlen(path) + strlen("mount -t nfs -o rsize=1024,wsize=1024,nolock : /opt/") +1);
					sprintf(cmd,"mount -t nfs -o rsize=1024,wsize=1024,nolock %s:%s /opt/",ip,path);
					free(ip);
					free(path);
					switch (fork())
					{
						...
						case 0:		
							/* child process */
							sleep(10);
							cprintf("NFS mount command:\n%s\n",cmd);
							system(cmd);
							_exit(0);
							break;
						default:	
							/* parent process should just continue his work */
				...
				/*
				 * Launches adslpolling_main in network.c
				 */
				start_adslpolling();
...
int 
version_notify_main(int argc, char **argv)
{

...
int ej_nvram_dslstatus_get(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	*/
	...
	if(!strcmp(arg, "dsmaxrate"))
		return websWrite(wp, "%d", sar_getstats("DS Max Attainable Bit Rate:"));
	if(!strcmp(arg, "usmaxrate"))
		return websWrite(wp, "%d", sar_getstats("US Max Attainable Bit Rate:"));

...
int ej_qos_config_setting(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if(!strcmp(arg,"qos_dns"))
	{
		char *qos_dns = nvram_safe_get("qos_app2");	
		if(((count == 1) && (!strcmp(qos_dns, "1"))) || ((count == 0) && (!strcmp(qos_dns, "0"))) || ((count == 2) && (!strcmp(qos_dns, "2"))))
			return websWrite(wp, "checked");
	}
	
	if(!strcmp(arg,"qos_telnet"))
	{
		char *qos_telnet = nvram_safe_get("qos_app3");	
	...
	{
		char *qos_smtp = nvram_safe_get("qos_app4");	
	...
	{
		char *qos_pop3 = nvram_safe_get("qos_app5");	
	...
	if(!strcmp(arg,"qos_spec_port3"))
	{
		char *qos_spec_port3 = nvram_safe_get("qos_spec_port3");	
		return websWrite(wp,"%s",qos_spec_port3);
	}
	
	if(!strcmp(arg,"qos_spec0"))
	{
		//char *qos_spec0 = nvram_safe_get("qos_spec0");	
		char *qos_spec0 = nvram_safe_get("qos_app6");	
	...
	if(!strcmp(arg,"qos_spec1"))
	{
		...
		char *qos_spec1 = nvram_safe_get("qos_app7");	
	...
	if(!strcmp(arg,"qos_spec2"))
	{
		...
		char *qos_spec2 = nvram_safe_get("qos_app8");	
		if(((count == 1) && (!strcmp(qos_spec2, "1"))) || ((count == 0) && (!strcmp(qos_spec2, "0"))) || ((count == 2) && (!strcmp(qos_spec2, "2"))))
			return websWrite(wp, "checked");
		
	}

	if(!strcmp(arg,"qos_spec3"))
	{
		//char *qos_spec3 = nvram_safe_get("qos_spec3");	
		char *qos_spec3 = nvram_safe_get("qos_app9");	
		if(((count == 1) && (!strcmp(qos_spec3, "1"))) || ((count == 0) && (!strcmp(qos_spec3, "0"))) || ((count == 2) && (!strcmp(qos_spec3, "2"))))
			return websWrite(wp, "checked");
...
{
	char *qosSW_l,*qos_ftp_l,*qos_http_l,*qos_dns_l,*qos_telnet_l,*qos_smtp_l,*qos_pop3_l,*qos_spec_port0_l,*qos_spec_port1_l,*qos_spec_port2_l,*qos_spec_port3_l,*qos_spec0_l,*qos_spec1_l,*qos_spec2_l,*qos_spec3_l,*qos_adv_l,*qos_size_l,*qos_vlan_l,*qos_vid_l/*,*vid_ip_l*/;
	...
	qos_dns_l = websGetVar(wp, "qos_dns", "0");	
	...
	if(!strcmp(qosSW_l,"1"))
	{
		...
		nvram_set("qos_app2", qos_dns_l);		
		nvram_set("qos_app3", qos_telnet_l);
		nvram_set("qos_app4", qos_smtp_l);
		nvram_set("qos_app5", qos_pop3_l);
		...
		nvram_set("qos_spec_port3", qos_spec_port3_l);		
		nvram_set("qos_app6", qos_spec0_l);
		nvram_set("qos_app7", qos_spec1_l);
		nvram_set("qos_app8", qos_spec2_l);
		nvram_set("qos_app9", qos_spec3_l);		

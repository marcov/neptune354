...
int
ej_get_simple_firmware_version(int eid, webs_t wp, int argc, char_t **argv)
{
	int ret = 0;
	ret += websWrite(wp,"%s %s <br/>", NEPTUNE_VERSION, MINOR_VERSION );
	ret += websWrite(wp,"Base source code: %s", CYBERTAN_VERSION);
	return ret;
}
...
ej_get_firmware_version(int eid, webs_t wp, int argc, char_t **argv)
{
	int ret = 0;
	ret += websWrite(wp,"%s %s &nbsp;&nbsp;&nbsp;<br/>", NEPTUNE_VERSION, MINOR_VERSION );
	ret += websWrite(wp,"Base source code: %s &nbsp;&nbsp;&nbsp;<br/>", CYBERTAN_VERSION);
	ret += websWrite(wp,"Uptime: %s &nbsp;&nbsp;&nbsp;<br/>", string_uptime() );
	ret += websWrite(wp,"%s ", string_load() );
	return ret;
}

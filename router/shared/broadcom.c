...
struct variable variables[] = {
	...
	{ "time_zone", "Time Zone", NULL, NULL, FALSE },
...
#ifdef DROPBEAR_SUPPORT
	{"dropbear_enable", "SSH server", validate_choice, ARGV("0", "1"), FALSE },
	//{"wan_ssh", "Accept ssh conn from WAN", validate_choice, ARGV("0", "1"), TRUE },
	{"dropbear_port", "ssh port listen", validate_range, ARGV("0", "65535"), TRUE },
#endif

	{"rwpart_enable", "Rw Partition", validate_choice, ARGV("0", "1"), FALSE },
	{"cush_enable", "Boot custom script", validate_choice, ARGV("0", "1"), FALSE },
	{"onsh_enable", "Boot custom script", validate_choice, ARGV("0", "1"), FALSE },
	
	//NFS
	{"nfsmount_enable", "Boot nfs mount", validate_choice, ARGV("0", "1"), FALSE },
	{"nfsmount_ip", "IP nfs mount", validate_ipaddr, ARGV("0", "1"), FALSE },
	{"nfsmount_path", "Path nfs mount", NULL, ARGV("0", "1"), FALSE },
 ...
 * 			4: sleep time 
 * 			5: action
 * 			6: funzione da chiamare (in broadcom.h)
 */
...
struct gozila_action gozila_actions[] = {
	...
	{ "Cloud",		"disconnect",	"",		5,	REFRESH,		disconnect_ipsec},
  	...
  	{"Log_incoming",        "pagerefresh",  "",     1,      REFRESH, syslog_refresh },
	{"Log_incoming",        "pagepre",      "",     1,      REFRESH, syslog_pagepre },
	{"Log_incoming",        "pagenext",     "",     1,      REFRESH, syslog_pagenext },
	{"Log_incoming",        "pageclear",    "",     1,      REFRESH, syslog_clear },
...
//wwzh add for DNS ENTRY
	...
	/*Demo*/
	{ "pippo" , "startpippo" , "" , 5 , REFRESH , pippo_start },
	{ "pippo" , "genkey" , "" , 5 , REFRESH , generate_sshkey },
	{ "pippo" , "createpart" , "" , 5 , REFRESH , create_partition },
...
//cjg
	...
	if(action == REFRESH)
	{
		...
		/*marco v*/
		else if(!strcmp(submit_button, "pippo") && !strcmp(submit_type, "genkey"))
		{
			websDone(wp, 200);
			goto last;
		}
		else if(!strcmp(submit_button, "pippo") && !strcmp(submit_type, "createpart"))
		{
			websDone(wp, 200);
			goto last;
		}
	  ...
	  char_t *url, char_t *path, char_t *query, int remote_flag)
{
    ...
    /*if change_action=="gozila_cgi" call gozila_cgi */
	...
	#endif
	#ifdef DROPBEAR_SUPPORT
		char dropbear_enable[4], *pt_dropbear;
		//char wan_ssh[4], *pt_wan_ssh;
		//char dropbear_port[8], *pt_dropbear_port;
	#endif
		
		char rwpart_enable[4], *pt_rwpart;
	...
	#endif
	#ifdef DROPBEAR_SUPPORT
		pt_dropbear = websGetVar(wp, "dropbear_enable", NULL);
		strcpy(dropbear_enable, pt_dropbear);
		
		//pt_wan_ssh = websGetVar(wp, "wan_ssh", NULL);
		strcpy(wan_ssh, pt_wan_ssh);
		
		//pt_dropbear_port = websGetVar(wp, "dropbear_port", NULL);
		//strcpy(dropbear_port, pt_dropbear_port);
	#endif
	/*rw partition creation...*/
		pt_rwpart = websGetVar(wp, "rwpart_enable", NULL);
		strcpy(rwpart_enable, pt_rwpart);
	...
	#endif
	#ifdef DROPBEAR_SUPPORT
		nvram_set("dropbear_enable", dropbear_enable);
		//nvram_set("wan_ssh", wan_ssh);
		//nvram_set("dropbear_port", dropbear_port);
	#endif
		
		/*Rwpart*/
		nvram_set("rwpart_enable", rwpart_enable);
		/*end*/
...
/*{Param1, Param2}
 * 
 * Param1 = function called in .asp pages like <% Param1(...); %>
 * 
 * Param2 = function as declared in broadcom.h or in this file.
 * 
 */
struct ej_handler ej_handlers[] = {
	...
	{ "get_simple_firmware_version", ej_get_simple_firmware_version },
...
/* for eight pvc*/
	...
	{ "dump_vpnlog", ej_dump_vpnlog },
	{"entry_logmsg",ej_entry_logmsg},
	{"vpn_logmsg",ej_vpn_logmsg},
	/*vpn-log*/
...
{ "ipppoe_support", ej_ipppoe_support },
	...
	/* Neptune */
#ifdef DROPBEAR_SUPPORT
	{"get_dbkey_status", ej_get_dbkey_status},
#endif
	...
	{"get_wanuptime", ej_get_wanuptime},
	{"show_macadd",ej_show_macadd},
	{"get_atuc", ej_get_atuc},
	{"get_dspver", ej_get_dspver},
	{"get_failtrain", ej_get_failtrain},
	{"get_freemem", ej_get_freemem},

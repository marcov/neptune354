...
//	wan_encapmode = websGetVar(wp, "SWwan_encapmode", NULL);
	...
	if((atoi(wan_status) == 0 ) || (atoi(wan_status) == 1 )){
		if (atoi(wan_status) == 0 )
	...
	foreach(p_config, nv_config, next)
	{
		if(p_ii == 0)
		{
			if((atoi(wan_status) == 0 ) && (atoi(fixedip) == 0 ))
				string_rep(p_config, 1, "1");
			else if(((atoi(wan_status) == 0 ) && (atoi(fixedip) == 1 )) || (atoi(wan_status) == 1)){
				...
				strcpy(p_config + (strrchr(p_config, ':') - p_config + 1), dns1);
			}
				...
				strncpy(p_config, p_config, (strrchr(p_config, ':') - p_config + 1));
				if ((atoi(wan_status) == 3 ) && (atoi(KaliveStatus) != 2))

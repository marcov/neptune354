...
#ifdef __TI_SQUASHFS_LZMA
                if((res = decompress_lzma_7z_buf((char *)cbuffer, size, buffer, &bytes)) == 0) {
                            BAD_ERROR("lzma::compress failed, unknown error %d\n", res);
                            return 0;
                }
#else                                            
...
#ifdef __TI_SQUASHFS_LZMA
		write_buffer->c_byte = mangle(write_buffer->data, file_buffer->data, file_buffer->size, block_size, noD, 1);
#else
...
#ifdef __TI_SQUASHFS_LZMA
		c_byte = mangle(write_buffer->data, file_buffer->data, file_buffer->size, block_size, noF, 1);
#else

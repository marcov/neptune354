...
squashfs_super_block *sBlk)
{
	...
	if(SQUASHFS_COMPRESSED(c_byte)) {
		...
		if(read_bytes(start + offset, c_byte, buffer) == FALSE)
			goto failed;
#ifdef __TI_SQUASHFS_LZMA
                if((res = decompress_lzma_7z_buf((const unsigned char *)buffer, c_byte, (unsigned char *)block, &bytes)) == 0) {
                        ERROR("lzma::compress failed, unknown error %d\n", res);
                        goto failed;
                }
#else
...
int read_data_block(long long start, unsigned int size, char *block)
{
	...
	if(SQUASHFS_COMPRESSED_BLOCK(size)) {
		if(read_bytes(start, c_byte, data) == FALSE)
			return 0;
#ifdef __TI_SQUASHFS_LZMA
                if((res = decompress_lzma_7z_buf((const unsigned char *)data, c_byte, (unsigned char *)block, &bytes)) == 0) {
                        ERROR("lzma::compress failed, unknown error %d\n", res);
                        return 0;
                }
#else
